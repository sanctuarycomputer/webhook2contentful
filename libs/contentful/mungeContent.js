  ...
  mungedType.controls.forEach(control => {
    ...
    if (Array.isArray(value)) {
      if (control.controlType === 'grid') {
        return (deserialized[control.name] = value
          .map(gridItem => {
      ...
      return (deserialized[control.name] = value.map(subvalue =>
        deserializeSingularValue(subvalue, control, result)
      ));
    }

    /* Handle Single Links & Primitives */
    return (deserialized[control.name] = deserializeSingularValue(
      value,
      control,
      result
    ));
...
module.exports = (result, config, mungedTypes) => {
  ...
  result.entries.forEach(entry => {
    ...
    if (config.singletons.includes(contentTypeKey)) {
      ...
      mungedContent[contentTypeKey][entry.sys.id] = deserialize(
        entry,
        mungedTypes,
        result
      );
